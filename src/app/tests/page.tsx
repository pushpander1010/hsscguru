...
export default function TestsPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [{ data: { user } }, { data: tests, error }] = await Promise.all([
    ...
    return (
      <AuthWrapper>
        <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
          <div className="card p-8 text-center">Loading tests...</div>
        </main>
      </AuthWrapper>
    );
  }
          ...
          <>
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-lg muted">
                    {user?.email ? `Welcome back, ${user.email}!` : "Welcome to Tests"}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="text-center">
                  <div className="text-3xl font-bold text-white">
                    {tests.filter(t => t.name.toLowerCase().includes('mock')).length}
                  </div>
                  <div className="text-sm muted">Mock Tests</div>
                </div>
                <div className="text-center">
                  <div className="text-3xl font-bold text-white">
                    {tests.filter(t => t.name.toLowerCase().includes('practice')).length}
                  </div>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {tests.map((test, index) => (
                <div key={test.slug} className="card p-6 border border-white/10 hover:border-white/20 transition-all duration-200 hover:scale-105 group">
                  {/* Test Header */}
                  <div className="flex items-start gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="title text-lg mb-2">{test.name}</h3>
                      <div className="text-sm muted">Code: {test.slug}</div>
                    </div>
                  </div>
                  ...
                  <div className="mb-4">
                    <span className={`
                      ...
                      ${test.name.toLowerCase().includes('mock') 
                        ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' 
                        : 'bg-green-500/20 text-green-400 border border-green-500/30'
                      }
                    `}>
                      {test.name.toLowerCase().includes('mock') ? 'Mock Test' : 'Practice Test'}
                    </span>
                  </div>
